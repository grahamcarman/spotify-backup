...
class SpotifyAPI:
    ...
    def authorize(client_id, scope):
        ...
        logging.info(
            f'Logging in (click if it doesn\'t open automatically): {url}')
        webbrowser.open(url)
    ...
    class _AuthorizationHandler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            # Read access_token and use an exception to kill the server listening...
                ...
                logging.info(
                    f'Received access token from Spotify: {access_token}')
...
def main():
    ...
    parser.add_argument('--format', default='json',
                        choices=['raw-json', 'json', 'txt'], help='output format (default: json)')
            ...
            for playlist in playlists:
                ...
                for track in playlist['tracks']:
                    ...
                    for artist in track['track']['artists']:
                        artists_pretty.append(artist['name'])

                    track_pretty = {
                        'name': track['track']['name'],
                        'artists': artists_pretty,
                        'album': track['track']['album']['name'],
                        'uri': track['track']['uri']
                    }
                    tracks_pretty.append(track_pretty)
                playlist_pretty = {
                    'playlist_name': playlist['name'],
                    'tracks': tracks_pretty
                }
                playlists_pretty.append(playlist_pretty)
            
            json_pretty = {
                'user': me['display_name'],
                'extract_date': datetime.today().strftime('%Y-%m-%d %H:%M:%S'),
                'playlists': playlists_pretty
            }
            json.dump(json_pretty, f)
